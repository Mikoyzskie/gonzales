import React from 'react'
import { righteous } from "@/app/font"

export default function Testimonial() {
    return (
        <section className="relative pb-24 lg:pb-20 overflow-hidden">
            <img
                className="absolute top-0 left-0"
                src="saturn-assets/images/testimonials/blue-light.png"
                alt=""
            />
            <img
                className="absolute top-0 right-0"
                src="saturn-assets/images/testimonials/orange-light.png"
                alt=""
            />
            <div className="relative container px-4 mx-auto">
                <div className="max-w-7xl mx-auto">
                    <div className="max-w-xl mb-24 mx-auto text-center">
                        <span className="inline-block py-1 px-3 mb-4 text-xs font-semibold text-orange-900 bg-orange-50 rounded-full">
                            TESTIMONIALS
                        </span>
                        <div className="font-heading text-4xl xs:text-6xl font-bold text-gray-900 mb-6 flex flex-col gap-5">
                            <p>Customer said about </p>
                            <h2 className={`madimi-one-regular`}>GONZALES FURNITURE</h2>
                        </div>
                    </div>
                </div>
            </div>
            <div className="relative">
                <div className="flex mb-8 justify-center">
                    <div className="w-full max-w-sm flex-shrink-0 mr-8">
                        <div className="p-6 bg-white rounded-4xl shadow-lg">
                            <div className="flex mb-6 items-center">
                                <img
                                    className="block w-15 h-15 rounded-full"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/avatar-group-4.png"
                                    alt=""
                                />
                                <div className="ml-5">
                                    <span className="block text-xl font-semibold leading-none">
                                        Marvin McKinney
                                    </span>
                                    <span className="block text-sm text-gray-500">
                                        CEO of Sans Design
                                    </span>
                                </div>
                            </div>
                            <p className="text-sm text-gray-900 mb-6">
                                Lorem ipsum dolor sit amet consectetur adipisicing elit. Sunt eligendi nostrum ut at eius aliquam voluptatum laboriosam, officiis voluptas possimus.
                            </p>
                            <div className="flex items-center">
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                            </div>
                        </div>
                    </div>
                    <div className="w-full max-w-sm flex-shrink-0 mr-8">
                        <div className="p-6 bg-white rounded-4xl shadow-lg">
                            <div className="flex mb-6 items-center">
                                <img
                                    className="block w-15 h-15 rounded-full"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/avatar-group-4.png"
                                    alt=""
                                />
                                <div className="ml-5">
                                    <span className="block text-xl font-semibold leading-none">
                                        Marvin McKinney
                                    </span>
                                    <span className="block text-sm text-gray-500">
                                        CEO of Sans Design
                                    </span>
                                </div>
                            </div>
                            <p className="text-sm text-gray-900 mb-6">
                                Lorem ipsum dolor sit amet consectetur adipisicing elit. Sunt eligendi nostrum ut at eius aliquam voluptatum laboriosam, officiis voluptas possimus.
                            </p>
                            <div className="flex items-center">
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                            </div>
                        </div>
                    </div>
                    <div className="w-full max-w-sm flex-shrink-0 mr-8">
                        <div className="p-6 bg-white rounded-4xl shadow-lg">
                            <div className="flex mb-6 items-center">
                                <img
                                    className="block w-15 h-15 rounded-full"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/avatar-group-4.png"
                                    alt=""
                                />
                                <div className="ml-5">
                                    <span className="block text-xl font-semibold leading-none">
                                        Jerome Bell
                                    </span>
                                    <span className="block text-sm text-gray-500">
                                        Founder of Uranus
                                    </span>
                                </div>
                            </div>
                            <p className="text-sm text-gray-900 mb-6">
                                Lorem ipsum dolor sit amet consectetur adipisicing elit. Sunt eligendi nostrum ut at eius aliquam voluptatum laboriosam, officiis voluptas possimus.
                            </p>
                            <div className="flex items-center">
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                            </div>
                        </div>
                    </div>
                    <div className="w-full max-w-sm flex-shrink-0 mr-8">
                        <div className="p-6 bg-white rounded-4xl shadow-lg">
                            <div className="flex mb-6 items-center">
                                <img
                                    className="block w-15 h-15 rounded-full"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/avatar-group-4.png"
                                    alt=""
                                />
                                <div className="ml-5">
                                    <span className="block text-xl font-semibold leading-none">
                                        Kathryn Murphy
                                    </span>
                                    <span className="block text-sm text-gray-500">
                                        CEO of Coca Soft
                                    </span>
                                </div>
                            </div>
                            <p className="text-sm text-gray-900 mb-6">
                                Lorem ipsum dolor sit amet consectetur adipisicing elit. Sunt eligendi nostrum ut at eius aliquam voluptatum laboriosam, officiis voluptas possimus.
                            </p>
                            <div className="flex items-center">
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                            </div>
                        </div>
                    </div>
                    <div className="w-full max-w-sm flex-shrink-0 mr-8">
                        <div className="p-6 bg-white rounded-4xl shadow-lg">
                            <div className="flex mb-6 items-center">
                                <img
                                    className="block w-15 h-15 rounded-full"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/avatar-group-4.png"
                                    alt=""
                                />
                                <div className="ml-5">
                                    <span className="block text-xl font-semibold leading-none">
                                        Kristin Watson
                                    </span>
                                    <span className="block text-sm text-gray-500">
                                        Founder of Neptunus
                                    </span>
                                </div>
                            </div>
                            <p className="text-sm text-gray-900 mb-6">
                                Lorem ipsum dolor sit amet consectetur adipisicing elit. Sunt eligendi nostrum ut at eius aliquam voluptatum laboriosam, officiis voluptas possimus.
                            </p>
                            <div className="flex items-center">
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                            </div>
                        </div>
                    </div>
                    <div className="w-full max-w-sm flex-shrink-0 mr-8">
                        <div className="p-6 bg-white rounded-4xl shadow-lg">
                            <div className="flex mb-6 items-center">
                                <img
                                    className="block w-15 h-15 rounded-full"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/avatar-group-4.png"
                                    alt=""
                                />
                                <div className="ml-5">
                                    <span className="block text-xl font-semibold leading-none">
                                        Marvin McKinney
                                    </span>
                                    <span className="block text-sm text-gray-500">
                                        CEO of Sans Design
                                    </span>
                                </div>
                            </div>
                            <p className="text-sm text-gray-900 mb-6">
                                Lorem ipsum dolor sit amet consectetur adipisicing elit. Sunt eligendi nostrum ut at eius aliquam voluptatum laboriosam, officiis voluptas possimus.
                            </p>
                            <div className="flex items-center">
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                            </div>
                        </div>
                    </div>
                </div>
                <div className="flex mb-15 justify-center">
                    <div className="w-full max-w-sm flex-shrink-0 mr-8">
                        <div className="p-6 bg-white rounded-4xl shadow-lg">
                            <div className="flex mb-6 items-center">
                                <img
                                    className="block w-15 h-15 rounded-full"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/avatar-group-4.png"
                                    alt=""
                                />
                                <div className="ml-5">
                                    <span className="block text-xl font-semibold leading-none">
                                        Marvin McKinney
                                    </span>
                                    <span className="block text-sm text-gray-500">
                                        CEO of Sans Design
                                    </span>
                                </div>
                            </div>
                            <p className="text-sm text-gray-900 mb-6">
                                Lorem ipsum dolor sit amet consectetur adipisicing elit. Sunt eligendi nostrum ut at eius aliquam voluptatum laboriosam, officiis voluptas possimus.
                            </p>
                            <div className="flex items-center">
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                            </div>
                        </div>
                    </div>
                    <div className="w-full max-w-sm flex-shrink-0 mr-8">
                        <div className="p-6 bg-white rounded-4xl shadow-lg">
                            <div className="flex mb-6 items-center">
                                <img
                                    className="block w-15 h-15 rounded-full"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/avatar-group-4.png"
                                    alt=""
                                />
                                <div className="ml-5">
                                    <span className="block text-xl font-semibold leading-none">
                                        Jerome Bell
                                    </span>
                                    <span className="block text-sm text-gray-500">
                                        Founder of Uranus
                                    </span>
                                </div>
                            </div>
                            <p className="text-sm text-gray-900 mb-6">
                                Lorem ipsum dolor sit amet consectetur adipisicing elit. Sunt eligendi nostrum ut at eius aliquam voluptatum laboriosam, officiis voluptas possimus.
                            </p>
                            <div className="flex items-center">
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                            </div>
                        </div>
                    </div>
                    <div className="w-full max-w-sm flex-shrink-0 mr-8">
                        <div className="p-6 bg-white rounded-4xl shadow-lg">
                            <div className="flex mb-6 items-center">
                                <img
                                    className="block w-15 h-15 rounded-full"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/avatar-group-4.png"
                                    alt=""
                                />
                                <div className="ml-5">
                                    <span className="block text-xl font-semibold leading-none">
                                        Albert Flores
                                    </span>
                                    <span className="block text-sm text-gray-500">
                                        CEO of Coca Soft
                                    </span>
                                </div>
                            </div>
                            <p className="text-sm text-gray-900 mb-6">
                                Lorem ipsum dolor sit amet consectetur adipisicing elit. Sunt eligendi nostrum ut at eius aliquam voluptatum laboriosam, officiis voluptas possimus.
                            </p>
                            <div className="flex items-center">
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                            </div>
                        </div>
                    </div>
                    <div className="w-full max-w-sm flex-shrink-0 mr-8">
                        <div className="p-6 bg-white rounded-4xl shadow-lg">
                            <div className="flex mb-6 items-center">
                                <img
                                    className="block w-15 h-15 rounded-full"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/avatar-group-4.png"
                                    alt=""
                                />
                                <div className="ml-5">
                                    <span className="block text-xl font-semibold leading-none">
                                        Kristin Watson
                                    </span>
                                    <span className="block text-sm text-gray-500">
                                        Founder of Neptunus
                                    </span>
                                </div>
                            </div>
                            <p className="text-sm text-gray-900 mb-6">
                                Lorem ipsum dolor sit amet consectetur adipisicing elit. Sunt eligendi nostrum ut at eius aliquam voluptatum laboriosam, officiis voluptas possimus.
                            </p>
                            <div className="flex items-center">
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                            </div>
                        </div>
                    </div>
                    <div className="w-full max-w-sm flex-shrink-0 mr-8">
                        <div className="p-6 bg-white rounded-4xl shadow-lg">
                            <div className="flex mb-6 items-center">
                                <img
                                    className="block w-15 h-15 rounded-full"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/avatar-group-4.png"
                                    alt=""
                                />
                                <div className="ml-5">
                                    <span className="block text-xl font-semibold leading-none">
                                        Brooklyn Simmons
                                    </span>
                                    <span className="block text-sm text-gray-500">
                                        CEO of Sans Design
                                    </span>
                                </div>
                            </div>
                            <p className="text-sm text-gray-900 mb-6">
                                Lorem ipsum dolor sit amet consectetur adipisicing elit. Sunt eligendi nostrum ut at eius aliquam voluptatum laboriosam, officiis voluptas possimus.
                            </p>
                            <div className="flex items-center">
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                                <img
                                    className="block mr-1.5"
                                    src="https://shuffle.dev/saturn-assets/images/testimonials/star.svg"
                                    alt=""
                                />
                            </div>
                        </div>
                    </div>
                </div>
                {/* <div className="px-5">
                    <div className="flex items-center justify-center">
                        <button className="inline-block mr-1 w-7 h-1 bg-orange-900" />
                        <button className="inline-block mr-1 w-7 h-1 bg-gray-400 hover:bg-orange-300" />
                        <button className="inline-block w-7 h-1 bg-gray-400 hover:bg-orange-300" />
                    </div>
                </div> */}
            </div>
        </section>

    )
}
